...
mu0 = np.linspace(0.0, 6.0, 10)
mu1 = np.linspace(-1.0, 1.0, 10)

k = 0
for i in range(len(mu0)):
    for j in range(len(mu1)):
        online_mu = (mu0[i], mu1[j])
        print("Parameter mu = ", online_mu)
        reduced_advection_dominated_problem.set_mu(online_mu)
        reduced_advection_dominated_problem.solve()
        reduced_advection_dominated_problem.export_solution(filename="solution_rb_advection", suffix=k)
        advection_dominated_problem.set_mu(online_mu)
        advection_dominated_problem.solve()
        advection_dominated_problem.export_solution(filename="solution_hf_advection", suffix=k)
        k += 1
